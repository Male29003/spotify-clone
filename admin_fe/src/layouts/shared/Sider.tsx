...
const AdminSider = () => {
    ...
    const { isExpanded } = useAdminSiderStore();
    ...
    return (
        <div className={`${isExpanded ? 'w-64' : 'w-20'} h-full bg-panel border-r border-border flex flex-col shrink-0 transition-all duration-300 ease-in-out`}>
            ...
            {/* Danh sách Menu */}
            <nav className={`p-4 flex flex-col gap-2 flex-1 custom-scrollbar ${isExpanded ? 'overflow-y-auto' : 'overflow-visible'}`}>
                { menu.map((item) => {
